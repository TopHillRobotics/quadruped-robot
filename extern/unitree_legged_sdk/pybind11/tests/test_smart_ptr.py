...
def test_unique_deleter():
    ...
    assert cstats4a.alive() == 2  # Two because of previous tests
    cstats4b = ConstructorStats.get(m.MyObject4b)
    assert cstats4b.alive() == 1
    del o
    assert cstats4a.alive() == 1  # Should now only be one leftover from previous tests
...
def test_holder_with_addressof_operator():
    # this tests must not throw exception from c++
