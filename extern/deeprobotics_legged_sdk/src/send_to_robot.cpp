...
void SendToRobot::work(){
  ...
  while (1){
    ...
    // cout << time_test.tv_nsec << endl;
    while(cmd_list.get_command_front(command_temp)){
...
void SendToRobot::set_send(RobotCmd& cmd){
  ...
  cmd_list.set_command(new Command(0x0111,sizeof(cmd), buffer));
}
void SendToRobot::all_joint_back_zero(){
  cmd_list.set_command(new Command(0x31010C05, 0,0));
}
void SendToRobot::robot_state_init(){
  cmd_list.set_command(new Command(0x31010C05, 0,0));
  usleep(1000*1000*7);
  cmd_list.set_command(new Command(0x0114, 0,0));//PS：超过5ms，未发数据set_send(cmd)，会失去控制权，要重新发送获取控制权
}
