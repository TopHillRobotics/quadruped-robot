...
#include "controllers/raibert_swing_leg_controller.h"
...
//check output
    EXPECT_NEAR(xyz(0), result(0),
                0.001);
    EXPECT_NEAR(xyz(1), result(1),
                0.001);
    EXPECT_NEAR(xyz(2), result(2),
                0.001);
}
...
//call function
    ...
    EXPECT_NEAR(action[0][0], get<0>(result)
    [0][0], 0.001);
    EXPECT_NEAR(action[0][1], get<0>(result)
    [0][1], 0.001);
    EXPECT_NEAR(action[0][2], get<0>(result)
    [0][2], 0.001);
    EXPECT_NEAR(action[0][3], get<0>(result)
    [0][3], 0.001);
    EXPECT_NEAR(action[0][4], get<0>(result)
    [0][4], 0.001);
    EXPECT_NEAR(footPosition[0], get<1>(result)
    [0], 0.001);
    EXPECT_NEAR(footPosition[1], get<1>(result)
    [1], 0.001);
    EXPECT_NEAR(footPosition[2], get<1>(result)
    [2], 0.001);
    EXPECT_NEAR(footTargetPosition[0], get<2>(result)
    [0], 0.001);
    EXPECT_NEAR(footTargetPosition[1], get<2>(result)
    [1], 0.001);
    EXPECT_NEAR(footTargetPosition[2], get<2>(result)
    [2], 0.001);
