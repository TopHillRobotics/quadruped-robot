...
qrWbcController<T>::~qrWbcController()
{
    ...
    typename std::vector<qrTask<T> *>::iterator iter = _task_list.begin();
...
template<typename T>
void qrWbcController<T>::run(void *precomputeData, std::vector<qrMotorCommand>& cmd)
{
    // MITTimer T1;
    ...
    if (_iter%2 == 0) {
        // MITTimer T1;
        // Update Model
        _UpdateModel(robot); // 0.05ms
        // printf("####UpdateModel SOLVE TIME: %.3f\n", T1.getMs());
        
        // MITTimer T2;
        // Task & Contact Update 0.003ms
        _ContactTaskUpdate(static_cast<qrWbcCtrlData*>(precomputeData));
        ...
        // TODO::
        // robot->state.visualizer.sa[2].Update(T1.getMs());
...
template<typename T>
void qrWbcController<T>::_UpdateLegCMD(std::vector<qrMotorCommand> &cmd)
{
    // std::vector<qrMotorCommand>& cmd = data->legCmd;
    // std::cout<< "****************************" << std::endl;
    // for (int i(0); i<12; ++i) {
    ...
    auto jointAngles = robot->state.motorAngles;
    for(size_t leg(0); leg < NumLeg; ++leg){
...
void qrWbcController<T>::_UpdateModel(qrRobot *robot)
{
    ...
    auto vBody = robot->state.baseVelocity;
    auto q = robot->state.motorAngles;
    auto dq = robot->state.motorVelocities;
    auto omegaBody = robot->state.baseRollPitchYawRate;
