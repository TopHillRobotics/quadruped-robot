...
namespace Quadruped {
    ...
    void ContactDetection::Reset(float currentTime)
    {
        //        
    }
    
    ContactDetection::ContactDetection(Robot* robotIn, GaitGenerator* gaitGeneratorIn, GroundSurfaceEstimator* groundEstimatorIn)
    : robot(robotIn), gaitGenerator(gaitGeneratorIn), groundEstimator(groundEstimatorIn)
    {
        ...
        for(int i=0; i<4; ++i) {
            windowFilter[i] = MovingWindowFilter<float, 3>(50);
            H[i][i] = 1.0; // torque
            H[i+4][i] = 1.0; // vz
            H[i+8][i] = 1.0; // pz
    ...
    void ContactDetection::Update(float currentTime)
    {
        ...
        for (int legId=0; legId<NumLeg; ++legId) {
            ...
            // std::cout << "dz [control frame] = " << dz[legId] << std::endl;
        ...
        // Eigen::Matrix<float,3,4> footVInControlFrame = groundEstimator->GetBaseFrameRInControlFrame() * footVInBaseFrame;
        for (int legId=0; legId<NumLeg; ++legId) {
        ...
        for(int legId=0; legId< NumLeg; ++legId) {
            ...
            // pContact(1, legId) = 0.5 * (1.0+std::erf((forces[legId]-meanTorques[legId])/(M_SQRT2 * sigmaTorques[legId])));
        }
        ...
        //     robot->footContact[footId] = isContact[footId];
        // }
        // std::cout << "[contactDetection] " << isContact.transpose() << std::endl;
        count++;
    }

    void ContactDetection::UpdateSlip(float currentTime)
    {
        // todo
    }
            
    void ContactDetection::GMObserver(float currentTime)
    {   
        // todo
    }

    Vec4<float> ContactDetection::JointObserver(float currentTime)
    {           
        ...
        // std::cout << "externalTorques[0] = " << externalTorques[0] <<std::endl;
        // std::cout << "dq " << robot->GetMotorVelocities() <<std::endl;
    ...
    ///
    ///
    WorkspaceDetection::WorkspaceDetection(Robot* robotIn, GroundSurfaceEstimator* groundEstimatorIn)
    ...
    Eigen::Matrix<float,3,4> WorkspaceDetection::Update()
    {
        ...
        Eigen::Matrix<float,3,4> hipPositionsInBaseFrame = robot->GetHipPositionsInBaseFrame();
    ...
    // class  WorkspaceDetect::random_workspace_detect()
    
    void WorkspaceDetection::Detect() 
    {
