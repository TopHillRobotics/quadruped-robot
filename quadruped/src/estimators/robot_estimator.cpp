...
namespace Quadruped {
    RobotEstimator::RobotEstimator(Robot *robotIn,
        ...
        : robot(robotIn), velocityEstimator(robotIn, gaitGeneratorIn, userParametersIn),
    ...
    void RobotEstimator::Update(float currentTime)
    {
        ...
        // auto & vis = robot->stateDataFlow.visualizer;
        // vis.datax.push_back(count++);
        // vis.datay1.push_back(estimatedVelocity[0]);
        // vis.datay2.push_back(estimatedVelocity[1]);
        // // vis.datay3.push_back(estimatedVelocity[2])
        // vis.sa[0].Update(estimatedVelocity[0]);
        // vis.sa[1].Update(estimatedVelocity[1]);

        // float dTime = ComputeDeltaTime(&robot->lowState);
        // CMUUpdate(dTime); // todo

        ComputeZMP();

        // Mat3<double> root_rot_mat = robot->stateDataFlow.baseRMat.cast<double>();
        // Vec3<double> vInBaseFrame = root_rot_mat.transpose()*x.segment<3>(3);
        // vis.datay3.push_back(vInBaseFrame[0]);
        // vis.datay4.push_back(vInBaseFrame[1]);
    }

    /** @brief Cart-On-Table Model: x_zmp = x_m-(z_m*dtdt_x_m)/(dtdt_z_m+g) */
    Vec3<float> RobotEstimator::ComputeZMP()
    {
    ...
    void RobotEstimator::CMUInitState()
    {
        // constructor
        eye3.setIdentity();
        // C is fixed
        ...
        // Q R are fixed
        Q.setIdentity();
        Q.block<3,3>(0,0) = 0.01*eye3;               // position transition
        Q.block<3,3>(3,3) = 0.01*eye3;               // velocity transition
        for (int i = 0; i < NumLeg; ++i) {
    ...
    void RobotEstimator::CMUUpdate(double dt)
    {
