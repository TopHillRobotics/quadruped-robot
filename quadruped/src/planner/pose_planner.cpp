...
namespace Quadruped {
PosePlanner::PosePlanner(Robot *robotIn, GaitGenerator *gaitGeneratorIn, StateEstimatorContainer<float>* stateEstimators)
    : robot(robotIn), gaitGenerator(gaitGeneratorIn), robotEstimator(stateEstimators->GetRobotEstimator()), groundEstimator(stateEstimators->GetGroundEstimator()) 
{
    /// leg contact status  ///
...
Vec6<float> PosePlanner::Update(float currentTime)
{
    ...
    rSP_ << 0, 0, 0 ;
    // std::cout << "rIB = " << rIB.transpose() << "rIF = " << rIF.transpose() << std::endl;
...
Eigen::MatrixXf PosePlanner::ComputeGradientG()
{
    ...
    int n=0;
    // printf("validContactPointId.size = %u\n", validContactPointId.size());
    for(int i : validContactPointId) {
        Vec3<float> gi = g[n];
        gi.normalize();
