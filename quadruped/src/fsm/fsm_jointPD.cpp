...
FSM_State_JointPD<T>::FSM_State_JointPD(ControlFSMData<T>* _controlFSMData)
    : FSM_State<T>(_controlFSMData, FSM_StateName::JOINT_PD, "JOINT_PD"), _ini_jpos(NumMotor){
  // Do nothing here yet
}
...
void FSM_State_JointPD<T>::OnEnter() {
  ...
  for(size_t leg(0); leg<4; ++leg){
    for(size_t jidx(0); jidx <3; ++jidx){
      _ini_jpos[3*leg + jidx] = FSM_State<T>::_data->desiredStateCommand->legJointq(jidx,leg);
    }
  }
}
...
void FSM_State_JointPD<T>::Run() {
  ...
  progress += 1.0/this->_data->userParameters->controlFrequency;
