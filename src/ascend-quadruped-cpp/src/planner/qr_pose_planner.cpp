...
: robot(robotIn), robotEstimator(robotEstimatorIn), groundEstimator(groundEstimatorIn), gaitGenerator(gaitGeneratorIn)
{
    ...
    rBH << 0.18, 0.18, -0.18, -0.18,
           -0.047, 0.047, -0.047, 0.047,
             0,    0,    0,    0;  // robot->config->defaultHipPosition;
...
Vec6<float> qrPosePlanner::Update(float currentTime)
{
    ...
    rIB_ = ProjectV(rIB);
    rBF = robot->state.GetFootPositionsInBaseFrame();
    ToCounterClockOrder(rBF);
    rIF = robot->state.GetFootPositionsInWorldFrame();
...
Eigen::MatrixXf qrPosePlanner::ComputeG()
{
    ...
    if (N==3){
        ...
        // printf("N < 3 !\n");
        std::cout << "force = "<<robot->state.footForce.transpose() << std::endl;
