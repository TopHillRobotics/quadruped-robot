...
using namespace std;
namespace Quadruped {
    WalkGaitGenerator::WalkGaitGenerator(Robot *robot,
    ...
    WalkGaitGenerator::WalkGaitGenerator(Robot *robot, string configFilePath)
    {
    ...
    void WalkGaitGenerator::Reset(float currentTime)
    {
    ...
    void WalkGaitGenerator::Update(float currentTime)
    {
        lastLegState = curLegState;
        count++;
        Eigen::Matrix<bool, 4, 1> contactState = robot->GetFootContacts();
        
        float augmentedTime, phaseInFullCycle, ratio;
        ...
        for (int legId = 0; legId < NumLeg; ++legId) {
            ...
            phaseInFullCycle = fmod(augmentedTime, fullCyclePeriod[legId]) / fullCyclePeriod[legId];
            std::cout << "phaseInFullCycle " << phaseInFullCycle << std::endl;
            
            ratio = dutyFactor[legId]; // initStateRadioInCycle[legId];
            if (phaseInFullCycle <= dutyFactor[legId]) { // stance
                ...
                legState[legId] = desiredLegState[legId];
                normalizedPhase(legId) = (phaseInFullCycle-ratio) / (1.0-ratio);
            }
            // SUB SWING
            if (desiredLegState[legId]== LegState::SWING) {
                ...
                float phaseInSwingCycle = (phaseInFullCycle-dutyFactor[legId]) / (1.0-dutyFactor[legId]);
            ...
            if (desiredLegState[legId] == SubLegState::TRUE_SWING && contactState[legId]) {
                ...
                detectedEventTickPhase[legId] = phaseInFullCycle;
        ...
        std::cout << "foot contact: " << robot->GetFootContacts().transpose() << std::endl;
