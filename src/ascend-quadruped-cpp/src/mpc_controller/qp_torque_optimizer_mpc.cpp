...
namespace Quadruped {
            ...
            Vec3<float> rpy)
    {
        ...
        Eigen::Matrix<float, 25, 25> AB= Eigen::Matrix<float, 25, 25>::Zero(25, 25);
        Eigen::Matrix<float, 13, 13> A= Eigen::Matrix<float, 13, 13>::Zero(13, 13);
        Eigen::Matrix<float, 13, 12> B= Eigen::Matrix<float, 13, 12>::Zero(13, 12);
        Eigen::Matrix<float, 13, 13> Adt= Eigen::Matrix<float, 13, 13>::Zero(13, 13);
        Eigen::Matrix<float, 13, 12> Bdt= Eigen::Matrix<float, 13, 12>::Zero(13, 12);
        Eigen::Matrix<float, 25, 25> ABexp= Eigen::Matrix<float, 25, 25>::Zero(25, 25);
        Eigen::Matrix<float, rowSizeAdtList,13> AdtList;
        Eigen::Matrix<float, sizeAqp, 13> Aqp= Eigen::Matrix<float, sizeAqp, 13>::Zero();
        Eigen::Matrix<float, sizeAqp, colSizeBqp> Bqp= Eigen::Matrix<float, sizeAqp, colSizeBqp>::Zero();
        ...
        A.block<3,3>(0, 6) = R.transpose(); //angular_velocity_to_rpy_rate;// R.transpose();
            ...
            float fMaxRatio)
    {
        ...
        Eigen::Matrix<float, 6, 3> Ablock = Eigen::Matrix<float, 6, 3>::Zero();
        Eigen::Matrix<float, 24, 1> lblock = Eigen::Matrix<float, 24, 1>::Zero();
        Eigen::Matrix<float, rowSizeAA, colSizeAA> AA = Eigen::Matrix<float, rowSizeAA, colSizeAA>::Zero();
        Eigen::Matrix<float, rowSizeAA, 1> lb = Eigen::Matrix<float, rowSizeAA, 1>::Zero();
            ...
            float regWeight)
    {
        ...
        for (int i=0; i<predHorizon; ++i) {
            ...
            // (roll, pitch, yaw)
            // std::cout << "dRPY" << drpy.transpose() << std::endl;
            // std::cout << "RPYdes" << RPYdes.transpose() << std::endl;
            desiredXs(i*13+0, 0) = RPYdes(0,0); //desiredX(0,0);
            desiredXs(i*13+1, 0) = RPYdes(1,0);//desiredX(1,0);
            desiredXs(i*13+2, 0) = RPYdes(2,0); //desiredX(2,0);
            // (x, y, z)
            // desiredX(4,0);
            desiredXs(i*13+3, 0) = desiredX(3,0); // predTimeStep * (i+1) * desiredX(9,0); 
            desiredXs(i*13+4, 0) = desiredX(4,0); // predTimeStep * (i+1) * desiredX(10,0); 
            desiredXs(i*13+5, 0) = desiredX(5,0);
            // (droll, dpitch, dyaw)
        ...
        Eigen::Matrix<float, desiredXsRowSize, desiredXsRowSize> Q = Eigen::Matrix<float, desiredXsRowSize, desiredXsRowSize>::Zero();
        Eigen::Matrix<float, 13, 13> Qblock = Eigen::Matrix<float, 13, 13>::Zero();
        for (int i=0; i<13; ++i) {
        ...
        Eigen::Matrix<float, Rsize, Rsize> R = Eigen::Matrix<float, Rsize, Rsize>::Identity();
        R = Eigen::Matrix<float, Rsize, Rsize>::Identity() * regWeight;
        Eigen::Matrix<float, Rsize, Rsize> quadTerm = Eigen::Matrix<float, Rsize, Rsize> ::Zero();
        Eigen::Matrix<float, Rsize, 1> linearTerm = Eigen::Matrix<float, Rsize, 1>::Zero();
                                                   ...
                                                   float fMaxRatio)
    {
        ...
        Eigen::Matrix<float, 3, 4> footInBaseFrame = quadruped->state.GetFootPositionsInBaseFrame();
        ...
        ABqp = Quadruped::ComputeMassMatrixMPC(quadruped->config->bodyMass,
                                            quadruped->config->bodyInertia,
        ...
        G = std::get<0>(Ga);
        a = std::get<1>(Ga);
        Eigen::Matrix<float, BqpSize, BqpSize> temp = Eigen::Matrix<float, BqpSize, BqpSize>::Identity();
        G = G; // + temp * 1e-6;
        // get the Ci and b
        ...
        CI = ComputeConstraintMatrixMPC(quadruped->config->bodyMass, contacts, frictionCoef, fMinRatio, fMaxRatio);
        Ci = std::get<0>(CI);
        bi = std::get<1>(CI);
