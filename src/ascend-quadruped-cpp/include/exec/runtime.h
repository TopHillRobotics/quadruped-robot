    ...
    while (countTime < 1000) { 
        ...
        if (countTime < 500) {
            ...
            if (typeid(T) == typeid(Robot)) {
                quadruped->yawOffset = quadruped->lowState.imu.rpy[2];
            }
            else {
                quadruped->yawOffset = static_cast<T*>(quadruped)->lowState.imu.rpy[2];                      
            }
            quadruped->basePosition = {0.f, 0.f, A1_BODY_HIGHT};
            quadruped->baseOrientation << 1.f, 0.f, 0.f, 0.f;
