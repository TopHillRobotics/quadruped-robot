...
=======
    ...
    EXPECT_EQ(footForce, robot->state.footForce);
...
// call function
    output = robot->state.ComputeJacobian(legId);
//check output: case, output
//    cout << "J: \n" << J << endl;
//    cout << "output: \n" << output << endl;
    EXPECT_NEAR(J(0, 0), output(0, 0),
...
<<<<<<< HEAD
// call function
    Eigen::Matrix<float, 3, 4> output = robot->config->defaultHipPosition;
//check output: case, output
...
// call function
    output = robot->state.MapContactForceToJointTorques(legId, contractForce);
//check output: case, output
=======
    // case 1 input
    legId = 2;
