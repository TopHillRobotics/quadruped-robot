...
class UnitreeDogEnv(robot_gazebo_env.RobotGazeboEnv):
    """Superclass for all CubeSingleDisk environments.
    """

    def __init__(self, ros_ws_abspath, initial_pose):
        """
        Initializes a new UnitreeDogEnv environment.
        if doesnt use controller_manager, therefore we wont reset the
        ...
        * /laser/scan: Laser Readings

        Actuators Topic List: /cmd_vel,

        Args:
        """
        rospy.logdebug("Start UnitreeDogEnv INIT...")
        # Variables that we give through the constructor.
        # None in this case

        # We launch the ROSlaunch that spawns the robot into the world
        #rospy.init_node('robot_env_A1', anonymous=True, log_level=rospy.ERROR)

        # Internal Vars
        # Doesnt have any accesibles
        self.controllers_list = ["trunk_imu"]

        # It doesnt use namespace
        self.robot_name_space = ""

        # We launch the init function of the Parent Class robot_gazebo_env.RobotGazeboEnv
        super(UnitreeDogEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        rospy.Subscriber("/legOdom", Odometry, self._odom_callback)
        rospy.Subscriber("/d435/depth/image_raw", Image, self._camera_depth_image_raw_callback)
        rospy.Subscriber("/trunk_imu", Imu, self._imu_callback)
        ...
        #dself.gazebo.pauseSim()

        rospy.logdebug("Finished UnitreeDogEnv INIT...")
    ...
    def _check_odom_ready(self):
        ...
        while self.odom is None and not rospy.is_shutdown():
            try:
                self.odom = rospy.wait_for_message("/legOdom", Odometry, timeout=5.0)
    ...
    def _check_camera_depth_image_raw_ready(self):
        ...
        while self.camera_depth_image_raw is None and not rospy.is_shutdown():
            try:
                self.camera_depth_image_raw = rospy.wait_for_message("/d435/depth/image_raw", Image, timeout=5.0)
    ...
    def has_crashed(self, min_laser_distance):
        ...
        if min_laser_distance != -1:
            data = self.get_camera_depth_image_raw()
            ...
            for i in range(data.height):
                if data.data[i][640] != 0 and data.data[i][640] < 0.5:
                    robot_has_crashed = True
                    break
